...
# ======= 각각의 Figure 생성 (코어 로직 동일) =======
# 1️⃣ 산점도
fig1, ax1 = plt.subplots(figsize=(8, 6))
sns.scatterplot(
    data=df, x="문화자원수", y="광역지자체 방문자 수", s=90, ax=ax1
)
...
ax1.plot(x_vals, y_vals, color="red", linestyle="--", label="회귀선")

texts = []
for i, row in df.iterrows():
    texts.append(
        ax1.text(row["문화자원수"], row["광역지자체 방문자 수"], row["광역지자체명"], fontsize=9)
    )
adjust_text(texts, ax=ax1, arrowprops=dict(arrowstyle="-", color='gray', lw=0.5))

ax1.set_xlabel("문화자원 수 (근대문화유산 + 지역축제 개수)")
ax1.set_ylabel("광역지자체 방문자 수")
ax1.set_title("문화자원 수가 많을수록 방문자수도 많다")
ax1.legend()
...
bars1 = ax2_1.bar(
    x - bar_width/2, 
    df_sorted["광역지자체 방문자 수"], bar_width, 
    label="방문자수(명)", color='tab:blue'
)
ax2_1.set_ylabel("방문자수(명)", color='tab:blue')
ax2_1.tick_params(axis='y', labelcolor='tab:blue')

ax2_2 = ax2_1.twinx()
norm_cultural = df_sorted["문화자원수"] / df_sorted["문화자원수"].max() * df_sorted["광역지자체 방문자 수"].max()
bars2 = ax2_2.bar(
    x + bar_width/2,
    norm_cultural, bar_width, 
    label="문화자원수 (정규화)", color='tab:orange', alpha=0.7
)
ax2_2.set_ylabel("문화자원수 (정규화, 최대 방문자수 기준)", color='tab:orange')
ax2_2.tick_params(axis='y', labelcolor='tab:orange')

ax2_1.set_xticks(x)
ax2_1.set_xticklabels(df_sorted["광역지자체명"], rotation=45)
ax2_1.set_title("광역지자체별 방문자수·문화자원수 이중 y축 비교")

legend_patches = [
    Patch(facecolor='tab:blue', label='방문자수(명)'),
    Patch(facecolor='tab:orange', label='문화자원수 (정규화)')
]
ax2_1.legend(handles=legend_patches, loc='upper right')

# ======= Streamlit 컬럼 배치로 나란히 표시 =======
...
with col2:
    st.header("2️⃣ 방문자수 & 문화자원수 막대그래프")
    st.pyplot(fig2)
